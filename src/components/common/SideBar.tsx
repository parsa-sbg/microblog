...
function SideBar({ user }: SideBarProps) {
    ...
    return (
        <div className={`${isMobileMenuOpen && 'translate-x-0 h-full'} absolute py-4 md:p-0 transition-all -translate-x-80 md:static md:!translate-x-0 rounded-md col-span-3 row-span-2`}>
            <aside className={` bg-main bg-opacity-30 h-full rounded-md p-4 min-w-44`}>
                ...
                <button className={`${!user && '!hidden'} md:hidden w-full text-left bg-gray-300 rounded-full transition-all py-1 px-4 hover:bg-gray-400`}>logout</button>
